...
def model_performance(project_name):
    ...
    if os.path.exists(results_path):
        ...
        with open(results_path, 'r', encoding='utf-8') as f:
            ...
            if reader:
                last = reader[-1]
                metrics['epochs'] = last.get('epoch', '-')
                metrics['train_loss'] = f"{float(last.get('train/box_loss',0))+float(last.get('train/cls_loss',0))+float(last.get('train/dfl_loss',0)):.4f}"
                metrics['val_loss'] = f"{float(last.get('val/box_loss',0))+float(last.get('val/cls_loss',0))+float(last.get('val/dfl_loss',0)):.4f}"
                metrics['map_50'] = last.get('metrics/mAP50(B)', '-')
                metrics['map_5095'] = last.get('metrics/mAP50-95(B)', '-')
                metrics['precision'] = last.get('metrics/precision(B)', '-')
                metrics['recall'] = last.get('metrics/recall(B)', '-')
