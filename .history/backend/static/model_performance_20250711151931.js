...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Keep Model button logic
  const keepBtn = document.getElementById('btn-keep-model');
  const feedback = document.getElementById('model-action-feedback');
  if (keepBtn) {
    keepBtn.addEventListener('click', function() {
      keepBtn.disabled = true;
      feedback.textContent = 'Saving model...';
      fetch(`/api/projects/${encodeURIComponent(project)}/keep_model`, {
        method: 'POST',
      ...
      .then(data => {
        if (data.success) {
          ...
          keepBtn.style.background = '#e8f5e9';
        } else {
          feedback.textContent = data.message || 'Failed to save model.';
          keepBtn.disabled = false;
        }
      })
      .catch(() => {
        feedback.textContent = 'Error saving model.';
        keepBtn.disabled = false;
      });
    });
  }
