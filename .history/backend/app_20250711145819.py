    ...
    if os.path.exists(results_path):
        with open(results_path, 'r', encoding='utf-8') as f:
            ...
            if reader:
                ...
                last_metrics['train_loss'] = f"{float(last.get('train/box_loss',0))+float(last.get('train/cls_loss',0))+float(last.get('train/dfl_loss',0)):.4f}"
                last_metrics['val_loss'] = f"{float(last.get('val/box_loss',0))+float(last.get('val/cls_loss',0))+float(last.get('val/dfl_loss',0)):.4f}"
                last_metrics['map_50'] = last.get('metrics/mAP50(B)', '-')
                last_metrics['map_5095'] = last.get('metrics/mAP50-95(B)', '-')
                last_metrics['precision'] = last.get('metrics/precision(B)', '-')
                last_metrics['recall'] = last.get('metrics/recall(B)', '-')
