...
# Model Performance Page (moved after app = Flask(__name__))

@app.route('/projects/<project_name>/model_performance')
def model_performance(project_name):
    # Path to results.csv (assuming single run for now)
    results_path = os.path.join(BASE_DIR, 'runs', 'detect', 'train', 'results.csv')
    epochs, train_loss, val_loss, map_50, map_5095, precision, recall = [], [], [], [], [], [], []
    last_metrics = {
        'last_epoch': '-',
        'last_train_loss': '-',
        'last_val_loss': '-',
    ...
    if os.path.exists(results_path):
        with open(results_path, 'r', encoding='utf-8') as f:
            ...
            for row in reader:
                # Some columns may be missing in early epochs, handle gracefully
                try:
                    epochs.append(int(row.get('epoch', len(epochs)+1)))
                    train_loss.append(round(float(row.get('train/box_loss',0))+float(row.get('train/cls_loss',0))+float(row.get('train/dfl_loss',0)), 4))
                    val_loss.append(round(float(row.get('val/box_loss',0))+float(row.get('val/cls_loss',0))+float(row.get('val/dfl_loss',0)), 4))
                    map_50.append(float(row.get('metrics/mAP50(B)', 0)))
                    map_5095.append(float(row.get('metrics/mAP50-95(B)', 0)))
            ...
            if reader:
                last = reader[-1]
                last_metrics['last_epoch'] = last.get('epoch', '-')
                last_metrics['last_train_loss'] = f"{float(last.get('train/box_loss',0))+float(last.get('train/cls_loss',0))+float(last.get('train/dfl_loss',0)):.4f}"
                last_metrics['last_val_loss'] = f"{float(last.get('val/box_loss',0))+float(last.get('val/cls_loss',0))+float(last.get('val/dfl_loss',0)):.4f}"
                last_metrics['last_map_50'] = last.get('metrics/mAP50(B)', '-')
                last_metrics['last_map_5095'] = last.get('metrics/mAP50-95(B)', '-')
                last_metrics['last_precision'] = last.get('metrics/precision(B)', '-')
                last_metrics['last_recall'] = last.get('metrics/recall(B)', '-')
    # Confusion matrix image (if exists)
    confusion_matrix_path = os.path.join('runs', 'detect', 'train', 'confusion_matrix.png')
    static_conf_matrix = os.path.join(BASE_DIR, 'static', confusion_matrix_path)
    if not os.path.exists(os.path.join(BASE_DIR, confusion_matrix_path)):
        confusion_matrix_path = None
    # Sample predictions (show up to 4 val_batch* images if exist)
    sample_predictions = []
    for fname in [f'val_batch{i}_labels.jpg' for i in range(4)] + [f'val_batch{i}_pred.jpg' for i in range(4)]:
        fpath = os.path.join(BASE_DIR, 'runs', 'detect', 'train', fname)
        if os.path.exists(fpath):
            sample_predictions.append(os.path.join('runs', 'detect', 'train', fname))
