...
def keep_model(project_name):
    ...
    # Determine base model name (e.g., yolov8) from results.csv or default
    base_model = 'yolov8'
    ...
    if os.path.exists(results_path):
        ...
        with open(results_path, 'r', encoding='utf-8') as f:
            ...
            if reader:
                # Try to get model_version from the first row if present, else default
                # (You can adjust this logic if you store model_version elsewhere)
                pass
    # Find next available version number
    version = 1
    while True:
        model_name = f"{base_model}_V{version}.pt"
    ...
    # Record model info (versioning, date, etc.)
    info_path = os.path.join(models_dir, f'{base_model}_V{version}_info.json')
    import datetime
    info = {
...
def get_model_versions():
    ...
    if project and family:
        ...
        if os.path.isdir(project_models_dir):
            versions += [f for f in os.listdir(project_models_dir) if f.lower().startswith(family.lower()) and f.lower().endswith(allowed_exts)]
    # Remove duplicates, preserve order (global first, then project-specific)
    seen = set()
    unique_versions = []
    for v in versions:
        if v not in seen:
            ...
            seen.add(v)
    return jsonify(unique_versions)
